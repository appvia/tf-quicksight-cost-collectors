...
resource "aws_cloudwatch_log_group" "usage_collector" {
  name              = "/aws/lambda/gitlab-usage-collector"
...
data "aws_iam_policy_document" "usage_collector" {
  ...
  statement {
    ...
    resources = ["arn:aws:secretsmanager:*:*:secret:${var.gitlab_token_secret_name}*"]
  }
  ...
  statement {
    ...
    resources = ["arn:aws:dynamodb:*:*:table/${var.user_data_dynamodb_table_name}"]
  }
}
